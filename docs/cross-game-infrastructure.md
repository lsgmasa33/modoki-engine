...
| Open the app's system Settings page | `engine/packages/capacitor-modoki-system` | `openAppSettings({ target: 'app' \| 'notifications' })` (#940): iOS `openNotificationSettingsURLString` (16+) or `openSettingsURLString`; Android `ACTION_APP_NOTIFICATION_SETTINGS` (8+) falling back to `ACTION_APPLICATION_DETAILS_SETTINGS`. Native because both Android pages are ACTION intents no URL can express. Resolves `{opened:false}` rather than rejecting. Consumer: wordweave's daily reminder row. Named for system surfaces so #1196's `openUrl` can land beside it |
