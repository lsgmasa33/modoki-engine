...
| AdMob (third-party plugin) | `@capacitor-community/admob`, per game | **Wordweave only so far** (#1309). Its SDK-neutral lifecycle, `games/wordweave/packages/app-services/src/adLifecycle.ts`, imports nothing but the engine and is written to be promoted as a file move when Court retargets (#1312). The AdMob adapter, the placements and the pacing numbers stay per game ([wordweave ads.md](../games/wordweave/docs/ads.md)) |
...
- **Settings + ad policy — deferred on evidence (#661, closed 2026-09-04).** The mechanism in each
  ...
  ⚠️ **The ad half's condition is now MET** (2026-09-17): wordweave has `runtime/adPolicy.ts`, a copy
  of Court's interstitial pacing (#932). It was copied, not promoted, because Court's own ads move to
  AdMob in #1312, and that is where both files get promoted together.
