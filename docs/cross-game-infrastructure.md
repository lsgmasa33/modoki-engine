...
| AppsFlyer attribution lifecycle | `engine/packages/modoki/src/runtime/core/attribution.ts` | Promoted in #1332 (2026-09-17), when Court's and Weaveling's `attribution.ts` had become byte-identical but for the bundle id. `createAttribution({ config, sdk, platform, bundleId })` owns the #458 init latch, the ATT call order (the only thing enforcing it since SDK v7), the consent retry rule, the iOS blank-`appleAppId` crash guard, `whenTrackingPromptSettled` and the advertising-id sentinel. ⚠️ **Takes the plugin AND `Capacitor` as parameters** — the plugin is vendored per game, and a game test's `vi.mock('@capacitor/core')` would not reach an engine-side import. Each game's `attribution.ts` is only the wiring; the rules are tested once in `tests/runtime/core/attribution.test.ts`, the wiring in each game's `services.test.ts` |
| AppLovin MAX plugin | `engine/packages/capacitor-applovin-max` | Promoted out of Court in #931 so wordweave could become the second consumer. **Parked, not unused:** on 2026-09-17 the owner chose AdMob first (#1309), so wordweave's ads went to `@capacitor-community/admob` instead, and MAX returns as mediator or demand source later. **The base plugin only**: `sdkKey`/`adUnitId` are call parameters, and the wrapper (`ads.ts`) and pacing policy (`adPolicy.ts`) stay per game. Court vendors it but keeps it PARKED out of `includePlugins` (#342). `games/3d-test` still carries a pre-promotion fork ([native-and-sdks.md](./native-and-sdks.md) § "Standalone Capacitor Plugin Pattern (iOS SPM)") |
| AdMob (third-party plugin) | `@capacitor-community/admob`, per game | **Both games** (#1309 wordweave, #1312 Court). The plugin is a per-game dependency, so each game keeps its own AdMob ADAPTER (`packages/app-services/src/ads.ts`: consent, unit ids, its analytics shapes), its placements and its pacing numbers ([wordweave ads.md](../games/wordweave/docs/ads.md), [Court ads.md](../games/court/ads.md)) |
| **Ad lifecycle + interstitial pacing** | `engine/packages/modoki/src/runtime/core/adLifecycle.ts` + `adPacing.ts` | Promoted in #1312 (2026-09-17), when Court became the second AdMob game. `adLifecycle.ts` is SDK-neutral and game-neutral: an `AdSdk` adapter plus `AdLifecycleHooks` in, and every concurrency and teardown rule Court paid for under MAX (the #458 latch, the #587/#611 reload blocker, the #631 restore) and Weaveling added for AdMob (presentation by events, reload on dismissal, the banner as desired state) out. `adPacing.ts` is `mayShowInterstitial` — tutorial, unlock, interval, No Ads, returning a REASON. ⚠️ **Carries no numbers and no copy**: each game passes its config. Weaveling adds its daily-board rule on top; Court keeps its coin economy in `runtime/adPolicy.ts` (the "two modules under one name" #1309 noted). **`adBreak.ts`** (#1329/#1330, written for both games at once) is the step between "an ad is allowed" and the ad: the No Ads offer's cooldown, the mid-level break's due check, and the offer/countdown reducer — pure, no numbers; each game owns the cards, stamps and clocks (`games/court/ads.md` § "Before the ad") |
| Leave the app for a system surface: its Settings page, or a web page | `engine/packages/capacitor-modoki-system` + the engine's `system.openUrl` action | `openAppSettings({ target: 'app' \| 'notifications' })` (#940): iOS `openNotificationSettingsURLString` (16+) or `openSettingsURLString`; Android `ACTION_APP_NOTIFICATION_SETTINGS` (8+) falling back to `ACTION_APPLICATION_DETAILS_SETTINGS`. Native because both Android pages are ACTION intents no URL can express. **`openUrl({ url })`** (#1196): https only, in Safari or the default Android browser. Games call it through the app-wide `system.openUrl` UI action ([ui-system.md](./ui-system.md) § "Engine built-in `UIAction`s"), not directly. Both methods resolve `{opened:false}` rather than rejecting. Consumers: **both** games. Wordweave uses both methods (the daily reminder row, and the settings footer's Privacy/Terms links); Court uses `openUrl` for the same footer |
