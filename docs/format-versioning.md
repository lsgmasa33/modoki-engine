...
⚠️ **Prefab's non-gate has since been REVERSED** (#1468, owner 2026-09-23) — see its row in § 3,
which keeps the old wording beside the new. Worth reading for what it says about this page as much
as about prefabs: a disposition is a judgement about a PREMISE, and prefab v5 changed the premise by
adding a field an older writer can destroy irrecoverably. Two passes confirmed the non-gate while
the premise held; neither was wrong. **A confirmed decision is not a permanent one, and recording
the reversal beside the original is how the next reader can tell the difference between a gap and a
change.**
...
| Prefab | `PREFAB_FORMAT_VERSION` (`runtime/core/version.ts`) | **REFUSE, on WRITE only** | ✅ (server-side) | ⚠️ **This REVERSES the row that stood here** (#1468, owner 2026-09-23). It read: *"— | raises only | nothing branches — a writer-only stamp, confirmed, not a gate to be invented (#365)"*. That was a considered decision twice over — #365 chose it and #784 phase C3 re-confirmed it — and it is kept above so nobody re-derives it as a gap. What changed is the PREMISE, not the judgement: the stamp was writer-only because no prefab field was worth protecting, and prefab v5 adds one that is — a minted `nodeGuid` per row, which an older serializer drops on its next save and which cannot be recovered, since re-minting produces different guids. **Disposition: refuse to SAVE, never to LOAD** (owner's ruling, on an explicit either/or: an old build must still OPEN a newer prefab and show what it can). Enforced SERVER-side at the byte-writing seam — `plugins/prefabWriteGuard.ts`, wired into `/api/write-file` and the asset scanner's `writeAssetGuid` — because a census found **17** paths by which a `.prefab.json` reaches disk and the editor-client wrapper covers 8; group B never touches the client at all. Four client-side writers refuse earlier, where a 409 would arrive after the live world had already been mutated: `applyToPrefabSelective`, `classifyExistingDocumentId` (which also stops a fresh guid being minted over an unreadable file), and the two import paths. `migrate-anchor-zindex.mjs` carries its own copy because it has no server and no wrapper. ⚠️ Every one of those comparisons is `version > CURRENT`, **never `!==`**: measured 2026-09-23, the authored corpus is 102 prefabs at v2 and 3 at v3 against a constant of 5, so an exact-match gate would refuse all 105. Still **no migration ladder and nothing version-gated on the LOADING path** — a row with no `nodeGuid` is one the next save mints for. **v6** (#1533) adds a nested row's `members` map, the second field the write gate earns its place on: an older build opens the file and shows the inner template through for those frames, but must not save over it and drop the rows. **v7** (#1543) adds a template reference node's `templateMoved`, the third: an older build opens the file without applying those moves, and must not save the node without them |
...
| `.particle.json` | `PARTICLE_FORMAT_VERSION` (`runtime/particles/types.ts`) | REFUSE | ✅ | not cached (`particleCache.ts`'s `failed` memo, marked permanent), logged at **error** — a "still loading" `null` and a "refused forever" `null` are otherwise identical to every consumer. `ParticleEditor` refuses to open it, disabling editing rather than substituting `defaultParticleEffect()` and marking it the saved baseline — which was #778's mechanism on this document (phase C2a) |
