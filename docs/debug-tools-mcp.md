...
- **Snapshot (`modoki_get_scene_state`):** **called bare it returns an INDEX** — per entity `id`,
  ...
  `contacts` (live solid `contacts` + sensor `overlaps`, GUIDs — a code-spawned partner has a runtime guid (#1210); `id:<n>` only for one with no guid — no EntityAttributes, or EntityAttributes added after spawn), `resources` (include resource entities,
