...
| [editor-panel-logic-tests-plan.md](./plans/editor-panel-logic-tests-plan.md) | Test the ~8.1k lines of decision logic trapped inside eight editor `.tsx` panels — extract-then-test (the `hierarchyFolders.ts` pattern), not jsdom panel mounting. Editor `.ts` is already 93% covered; the `.tsx` half is 68% untouched. Phase 0 replaces the `grep` proxy with real V8 coverage first |
