...
| [mcp-surface-audit-plan.md](./plans/mcp-surface-audit-plan.md) | **In-flight audit tracker** — the 2026-08-21 consistency/usability pass over all three agent surfaces, run against the rules [mcp-tool-conventions.md](./mcp-tool-conventions.md) established. Diagnosis: `contracts.ts` has an OBSERVED half (`method`/`route`/`op`, asserted) and a DECLARED-ONLY half (`varies`/`undoable`/`persists`/…, unchecked), and every drift found lives in the unchecked half — including a live rank-1 false success in `modoki_hit_regions`. Carries the measured #261 recommendation (a React-commit relayout settles in ZERO frames, so the transient is FlexLayout-specific and the refusal is true at the instant it is made) |
...
| [clones-and-ports.md](./clones-and-ports.md) | The multi-clone/multi-machine setup — the clone table, sync recipes, and the full reasoning behind RULE 1 (per-clone install/build, #215) and RULE 2 (derived Vite/CDP ports) |
...
| [profiler.md](./plans/profiler.md) | **Carries the #238 boot-stall handoff for a cold session** (Phase 2, "⇥ HANDOFF") — what the stall is, the three still-unexplained projects, the probe scripts, and the traps that have cost time. The Modoki Profiler — answer "where did the frame go?" for a human at a panel AND for Claude over MCP, from ONE data model, on desktop and on a phone. Marker hierarchy + engine instrumentation + aggregation (the spine), then the agent surface, the Profiler panel, frame capture, GPU timestamp queries and memory. Compared against Unity's Profiler; deliberately does NOT copy deep profiling or a multi-thread flame chart |
...
| [ui-scroll-view-plan.md](./plans/ui-scroll-view-plan.md) | Plan for #250 — a UI scroll view whose content is a recycled pool of prefab **entries**, covering all three cases in one model (one entry at a time / many along an axis / a 2-D grid) from a `countX × countY` index space. Records the engine-writes-content choice, the padding-offset geometry, why the scroll write must not dirty the UI tree, and Court's level selector as the One-case migration. **All 11 steps landed** (#250 + #316, 2026-08-21); kept as the record of the design rationale and two still-open questions, pending a fold into ui-system.md |
