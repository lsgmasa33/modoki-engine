...
| Doc | What it is |
|---|---|
| [low-end-device-support.md](./plans/low-end-device-support.md) | **Rewritten 2026-08-12** (3,901 → ~440 lines; the superseded evidence is in git at `8bed9661`). Tracks four goals: detect the device, gate features per tier, tune parameters per tier, and build the tools that make things faster. Goals 2 and 3 are DONE and folded into [rendering.md](./rendering.md) (#121/#188/#202, closed). The live decision is the **three-layer resolver** — GPU family+generation, then a vendored GPU database, then the probe as an isolated fallback — which replaces a boot benchmark that was measured to swing 3× per project and could not reach its own top band. Goal 4 (texture LOD by tier, real LOD chains) has never started and is where the frame rate actually is |
| [draw-call-instancing-plan.md](./plans/draw-call-instancing-plan.md) | **#154 P4b — BUILT, MEASURED, AND DEFAULT OFF; read §9 first.** The mechanism works (197 draw calls removed, 235 → 38) and the frame does not move: sling is **GPU-bound at ~85 ms**, and the 25 ms `submit` marker that motivated the whole plan was a CPU-side wait for a GPU that was already the limiter. §§1-8 are the refuted reasoning, preserved deliberately. The durable output is the instrumentation, not the optimization — plus the finding that 183 of sling's meshes carry a single-level LOD chain, so triangle count cannot be cut until real chains exist |
