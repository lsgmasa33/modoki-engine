...
| Doc | What it is |
|---|---|
| [mcp-definition-surface.md](./plans/mcp-definition-surface.md) | Cut what the MCP tool surface costs an agent without losing a tool or a description that earns its length. Carries the 2026-08-31 measurement (248,755 chars / 148 tools), the finding that Claude Code loads it **deferred** (names only, ~4,665 chars) so #456's per-turn premise no longer holds there, and the real cost — `tools/list` churn invalidating the whole prompt-cache prefix. Measurement revised the plan's own thesis: the reload-flap is rare (sub-second outages against a 5s poll), while opening/closing a project changes the tool list **every time** and is untouched by the hysteresis. Phase 1 measured + documented, Phase 2 landed; the stable-pair redesign is an open owner decision. `$defs` dedup and broad consolidation explicitly declined |
