...
| [device-authoring-parity-plan.md](./plans/device-authoring-parity-plan.md) | Closing the WRITE gap on the `device_*` surface (#166) — why eval could never substitute (both eval APIs are generated from the op registry, so eval adds composition and zero capability), the measured editor↔device op gap, and the structural rule that keeps it closed: a runtime-only op registers in `agentBridge`, where both surfaces get it. P1–P5 landed; P6 (`modoki_profiler` — the profiler had no typed tool on EITHER surface) open |
...
| [sling-lod-plan.md](./plans/sling-lod-plan.md) | **#154 P4b follow-on** — get `games/sling` to 30 fps on the Huawei Y6. Arrived at by ELIMINATION: the board costs ~35 ms of a 76 ms frame (A/B/A, board hidden), and removing 84% of its draw calls recovers none of it, so the cost is geometry. The LOD system is already built and wired; two config facts stop it firing — the field tiles are baked `lodCount: 1`, and the chains that exist switch at 80/250 world units on a board a few units across. Carries the measurement harness and the three guards that each caught a wrong number |
...
| [court-art-direction.md](./plans/court-art-direction.md) | Court's art-direction brief + generation prompt pack — "a beautiful chess set in a calm, expensive place": the opaque stone slab that owns its lighting (so meaning-colour never sits on a photo), the three registers, and the per-asset prompts. ⚠️ **REFERENCE, not a plan to execute** — the pass is run iteratively (#58); §9's order of work is dead and a ruling here can be stale. Read it for the measured findings |
