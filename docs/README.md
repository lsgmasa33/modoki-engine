...
| Doc | What it is |
|---|---|
| [mcp-five-capability-gaps-plan.md](./plans/mcp-five-capability-gaps-plan.md) | Phased plan for #288 — five engine capabilities with no agent tool (physics scene queries, pose-a-clip-at-a-time, delete an asset, PlayerPrefs, and the agent-unreachable "+ New X" save dialog), plus device parity and the un-exported `VideoCache` singleton. Written against [mcp-tool-conventions.md](./mcp-tool-conventions.md); carries a handoff section and the ⚠️ CORRECTED blocks recording three designs the code disproved (a `create`-override scene kind that discards the live world, an un-extractable pose path, a stale asset manifest). Not started |
