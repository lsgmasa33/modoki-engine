...
| [debug-release-build-separation-plan.md](./plans/debug-release-build-separation-plan.md) | Make `build.debugBuild` the SINGLE source of truth for debug vs release. Today the JS surfaces key on the project flag while every native gate keys on the Xcode/Gradle configuration, so `debugBuild:true` + a Release configuration silently produces a debug build that cannot debug. Unify the gates, refuse a distribution build with the flag on, and guard the stripping with a bundle-content test |
...
| [court-art-direction.md](./plans/court-art-direction.md) | Court's art-direction brief + generation prompt pack — "a beautiful chess set in a calm, expensive place": the opaque stone slab that owns its lighting (so meaning-colour never sits on a photo), the three registers, and the per-asset prompts. **Direction agreed 2026-07-31; art not yet produced** |
