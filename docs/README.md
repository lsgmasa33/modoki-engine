...
| [cross-game-shared-infra-plan.md](./plans/cross-game-shared-infra-plan.md) | Plan (tracked by #632) to extract Court's main menu / level selector / IAP / AppsFlyer / Firebase / daily-challenge mechanism into shared `engine/packages/` (behavior fixed once) while keeping content (art, product catalog, level lists) per-game; carries the audited current-state table and the decided distribution mechanism (TS via `@modoki/engine`, native via the vendoring pipeline, git-URL/registry rejected) — AppsFlyer's move is NOT settled, it's blocked on that decision (nothing landed yet) |
