...
| [iap.md](./iap.md) | In-app purchases & subscriptions — the two invariants (grant durably THEN finish; every grant idempotent by transaction id) and the crash matrix they prove, the `StoreBackend` port, product-kind-decides-who-owns-truth, local on-device verification and what it costs, the first-party `capacitor-modoki-iap` plugin and why an off-the-shelf one cannot work, iOS StoreKit Testing (Xcode's Run SYNCS the config — it is one-time setup, not per-launch), Play Billing gotchas, and the on-device interruption harness that makes the force-quit test performable |
...
| Doc | What it is |
|---|---|
| [low-end-device-support.md](./plans/low-end-device-support.md) | **Now the COST half** (#154): draw-call batching/instancing, the per-project pass to 30 fps, and perf guardrails. The correctness half is DONE and folded into [rendering.md](./rendering.md) — capability probe, GPU context-loss recovery, and quality tiers (#121, closed). Read its §0 handover first: §0b holds the live measurements and is the authority over the dead figures still sitting in §4 |
| [draw-call-instancing-plan.md](./plans/draw-call-instancing-plan.md) | **#154 P4b** — collapse repeated (mesh, material) draws into instanced draws. Written after the measurement that justifies it: sling holds 84 ms / 11.9 fps on the Y6 with `submit` at 25 ms across 235 draw calls and the GPU idle 17.7 ms, i.e. CPU-bound on draw SUBMISSION — so LOD/decimation (which cut triangles) would not touch it. Carries the automatic-vs-explicit fork, the batch-key trap (light masks + colour overrides break a shared material), and the A/B/A discipline P4a cost us |
| [profiler.md](./plans/profiler.md) | The Modoki Profiler — answer "where did the frame go?" for a human at a panel AND for Claude over MCP, from ONE data model, on desktop and on a phone. Marker hierarchy + engine instrumentation + aggregation (the spine), then the agent surface, the Profiler panel, frame capture, GPU timestamp queries and memory. Compared against Unity's Profiler; deliberately does NOT copy deep profiling or a multi-thread flame chart |
...
| [court-tray-readability-plan.md](./plans/court-tray-readability-plan.md) | Tracker — Court's tray badge (#171), and **the only art-direction record Court has** since `court-art-direction.md` was retired 2026-08-08 (its surviving measurements are absorbed into a section here, with a ledger of what that brief specified and nobody built). Carries two general lessons worth reading outside Court: `Text2D.outlineWidth` is a NORMALISED SDF value, so a halo that works at cell size is ~1 device px at tray size; and a tint MULTIPLIES a white master, so a dark tint deletes the art it is tinting. Phase A/B mostly landed + the tray layout is now prefab-authored; Phases C/D/E open |
| [court-test-cost-plan.md](./plans/court-test-cost-plan.md) | Tracker — making Court's suite survive **2000 levels**. Measured: one 490-second `it()` that no sharding can split, thirteen ungated files that load the corpus on every `npm test` (ten of them parse 167 files to use ONE), and sweeps that are O(levels × 11 board kinds). Phase 2 is only possible because a level's id is now its content hash; explains why a committed proof-ledger was rejected for git |
