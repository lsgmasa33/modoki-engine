...
| [format-versioning.md](./format-versioning.md) | **Normative** — one design for every versioned persisted document (scene, prefab, sidecar, manifests, OTA, saves). The three KINDS of version (format vs data-identity vs compatibility) and why conflating them picks the wrong failure behaviour; the rule that a build must never overwrite a document it cannot read; the three contracts (classification and stamping are shared, the refusal channel is deliberately local); the decision table listing every versioned document, whether anything reads it, and what it does on too-new; and the two ways a guard in this area steers a fix wrong |
