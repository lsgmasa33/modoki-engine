...
**Generalised — the rule those two are instances of: never free a GPU-backed resource inside the pass that then renders.** `Scene2DRenderer.renderFrame` ends by calling `pool.renderAll`, so anything destroyed earlier in that same pass is destroyed *while the renderer is still holding it*. Three places in the 2D path now obey this, and they were arrived at INDEPENDENTLY, from three different symptoms — worth reading as one rule rather than three coincidences: mask teardown defers to `pendingMaskDestroy`, drained at the top of the next frame (#455); `releaseSpriteTexture` defers its `Assets.unload` by a macrotask and cancels it on re-retain (the same-frame rebuild case); and `disposeSlot` uses a bare `Shader.destroy()` / `texture.destroy(false)` so shared programs and Assets-owned sources outlive the slot. The risk ranks by whether the renderer reaches the doomed object from OUTSIDE the parent/child walk: a mask (an effect, plus a cached bind group) is the dangerous end, an already-`removeFromParent()`ed sprite is the safe end.
