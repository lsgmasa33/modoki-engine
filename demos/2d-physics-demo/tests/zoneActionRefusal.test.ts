...
import { createTestWorld, dispatchUIAction, isActionRefusal, Renderable2D } from '@modoki/engine/runtime';
...
const CASES = [
  { action: 'sensorZone/enter', event: 'zone', phase: 'enter' },
  { action: 'sensorZone/exit', event: 'zone', phase: 'exit' },
  { action: 'triggerZone/enter', event: 'zoneTrigger', phase: 'enter' },
  { action: 'triggerZone/exit', event: 'zoneTrigger', phase: 'exit' },
] as const;

describe('2d-physics-demo — zone reactions without params.self refuse (#1185)', () => {
  ...
  it.each(CASES)('ACCEPT: $action WITH self returns no refusal and journals the $phase', async ({ action, event, phase }) => {
    tw = createTestWorld({});
    await game.registerSystems?.();
    const zone = tw.spawn(Renderable2D({ color: 0x123456, opacity: 0.25 }));
    const body = tw.spawn(Renderable2D({ color: 0xffffff, opacity: 1 }));
