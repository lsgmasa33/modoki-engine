/**
 * Generic curve/color-gradient types + pure sampling helpers — no THREE, no GPU, fully
 * unit-testable. Used by the particle schema's over-life modifiers and spawn values, and by
 * `MaterialInstance`'s curve-driven parameters. Generic enough that neither consumer should own
 * it — that's why it's L0 core rather than living under `particles/`. Numbers that vary use
 * {@link MinMax} (uniform random between min/max); properties that animate over 0..1 use
 * {@link Curve} (piecewise-linear) or {@link Gradient} (color/alpha).
 */
