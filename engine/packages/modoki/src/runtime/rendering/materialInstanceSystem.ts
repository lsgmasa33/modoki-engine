...
import { getSimDelta, getVisualDelta } from '../core/getTime';
import { getEntityObjects } from './materialBroker';
...
type Broker2DModule = typeof import('./sprite2DMaterialBroker');
...
if (__MODOKI_MODULE_RENDER2D__) {
  void import('./sprite2DMaterialBroker').then((m) => { broker2D = m; });
}
import { applyPropOverride } from './materialInstanceClones';
import { materialProvider } from './materialProvider';
function resolveMaterial(materialRef: string) { return materialProvider.get()?.resolveMaterial(materialRef); }
