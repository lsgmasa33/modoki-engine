...
// -- Atlas (the authored `.atlas.json`: member sprites + pack options). The DERIVED bookkeeping
...
const ATLAS_FIELDS: FieldMeta[] = [
  ...
  { key: 'extrude', type: 'number', default: 1, min: 0, note: 'edge-bleed in px replicated outward from each sprite; adds to its reserved footprint' },
  { key: 'maxPages', type: 'number', min: 0, note: 'OPTIONAL cap on generated pages; members that do not fit are reported as overflow' },
  // ⚠️ Named here because a writer that does not know about it DELETES it. An add/remove-member
  // round-trip once dropped this whole block (QA-ASSET-0013) — the settings deciding how the
  // packed page is ENCODED — with nothing erroring and the members list looking right.
  { key: 'texture', type: 'object', note: 'OPTIONAL per-atlas page encoding override { format, maxSize, mipmaps, wrapS, wrapT, colorspace }. Defaults to WebP - KTX2 emits no 2D variant, which the PixiJS path needs.' },
];

export function defaultAtlasFile(): Record<string, unknown> {
  return { ...defaultAtlasSource() };
}
...
const SCHEMAS: Record<AssetSchemaType, () => AssetSchema> = {
  ...
  atlas: () => ({ type: 'atlas', fields: ATLAS_FIELDS, example: defaultAtlasFile(), notes: 'The AUTHORED half of a sprite atlas - which sprites go in it and how they pack. The packed result (pages, frame map, hash) is DERIVED, lives in the .meta.json sidecar, and is regenerated by a re-pack (modoki_reimport_asset); never write it here. `members` are sprite-slice GUIDs, never literal paths. `pageSize` must be a multiple of 4. Editing this document does not re-pack - the pages on disk stay stale until a re-pack runs.' }),
  animset: () => ({ type: 'animset', fields: ANIMSET_FIELDS, example: defaultAnimSetFile(), notes: 'A named set of playback clips for an animated model, referenced by trait fields that pick a clip BY NAME. No `version` field on this type — do not add one. `source` is an optional model GUID: an AnimationLibrary merges its clips from that model, and a scene referencing the animset loads that model before it goes live; `clips[].name` must match a clip baked into that model\'s animation bank. `id` assigned on save if absent.' }),
};
...
export function defaultAssetData(type: AssetSchemaType): unknown {
  ...
  if (type === 'atlas') return defaultAtlasFile();
...
export function validateAssetData(type: AssetSchemaType, data: unknown): { errors: string[]; warnings: string[] } {
  ...
  if (type === 'atlas') {
    // `members` is the one field the packer walks unconditionally, so a non-array is a hard
    // error. Everything else is advisory: an atlas with no pack options still packs at the
    // defaults. `pageSize` NOT being a multiple of 4 is the one that renders solid black on
