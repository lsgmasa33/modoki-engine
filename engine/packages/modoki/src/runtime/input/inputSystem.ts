...
import { DIGITAL, createInputFrame, beginSample, clampAxes, computeEdges, computePointerEdge, makeFlags, type InputFrame, type FlagMap, type InputDevice } from '../core/inputActions';
import { sampleAll } from './inputSources';
import { getPresentationScale } from './presentationScale';
...
export function inputSystem(world: World): void {
  ...
  world.query(Input).updateEach(([inp]: [InputFrame]) => {
    ...
    // Positions (x/y/startX/startY) stay RAW viewport CSS px — ratio-matched to
    // getBoundingClientRect, already zoom-invariant for raycast/hit-testing (do not scale).
    // dragX/dragY are a MAGNITUDE, scaled here (once, at the merge point) to zoom-0-equivalent
    // px, so every reader of Input.pointer.dragX/dragY — the pointerDrag accessor or a direct
    // field read — sees the same presentation-invariant value. See presentationScale.ts.
