...
export function MaterialBatchView({ paths }: { paths: string[] }) {
  ...
  return (
    <>
      {parkBanner}
      {banner}
      <ColorField label="Color" dataUiId="assetView.materialBatch.color" value={(merged.color as number) ?? DEFAULT_COLOR} mixed={isMixed('color')} onChange={(v) => writeFieldAll('color', v)} />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
        ...
        <MixedCheckbox checked={!!merged.transparent} mixed={isMixed('transparent')} onChange={(v) => writeFieldAll('transparent', v)} dataUiId="assetView.materialBatch.transparent" dataUiLabel="Transparent" />
      </div>
      ...
      <DropdownField label="Side" dataUiId="assetView.materialBatch.side" value={(merged.side as string) ?? 'front'} mixed={isMixed('side')} options={['front', 'double', 'back']} onChange={(v) => writeFieldAll('side', v)} />
      {!isUnlit && <ColorField label="Emissive" dataUiId="assetView.materialBatch.emissive" value={(merged.emissive as number) ?? 0} mixed={isMixed('emissive')} onChange={(v) => writeFieldAll('emissive', v)} />}
