...
import {
  hasTimelinePreviewSession, endTimelinePreviewSession, isPreviewRestoreInFlight, cancelPreviewGestures, whenPreviewRestoresLanded,
...
export async function enterPlay(): Promise<void> {
  ...
  if (lastRestoreFailed()) {
    console.warn('[Editor] Play refused — the last Play/preview restore FAILED, so the live world may not be the authored one. Reload the scene first.');
    return;
  }
...
export async function stopPlay(): Promise<void> {
  ...
  try {
    ...
    if (snap.key !== currentSceneKey()) return;
 ...
 *  reaches its snapshot in the same synchronous run it always did, instead of yielding first. */
function takeDownPreviewEnvelope(): Promise<unknown> | null {
  cancelPreviewGestures();
  cancelPendingPreviewBegins();
  const ending = hasTimelinePreviewSession() ? endTimelinePreviewSession({ restore: true })
    : isPreviewRestoreInFlight() ? whenPreviewRestoresLanded()
      : null;
  const m = getRunMode();
