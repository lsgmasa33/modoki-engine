...
export function ensureCapacitorDeps(projectRoot: string, platform: NativePlatform, editorRoot: string): ScaffoldResult {
  ...
  const want: Record<string, string> = {
    ...
    // capacitor-game-debug is a vendored engine plugin, not a registry package — capDepRange
    // above would have no editor-pinned version to find and fall back to the (meaningless, for a
    // local plugin) core Capacitor range. It gets a placeholder spec instead; vendorEnginePlugins
    // rewrites it to file:plugins/<name>-<ver>.tgz (a copy) before install.
