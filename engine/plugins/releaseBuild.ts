...
/** The build-number argument appended to an xcodebuild build/archive command (#1226). A command-line
 *  build setting overrides the committed `CURRENT_PROJECT_VERSION`, which `Info.plist` reads as
 ...
 *  `android/.gradle/`, which every Capacitor `android/.gitignore` ignores — it is regenerated per build
 *  and must never show up in `git status`, which is the whole point of #1226. */
export const ANDROID_VERSION_CODE_INIT_SCRIPT_PATH = 'android/.gradle/modoki-version-code.init.gradle';

/** The per-platform arguments files a hand-run build appends (`writeBuildNumberArgFiles`,
 *  healNativeConfig.ts). Both inside gitignored build/cache folders — `ios/App/build/` is where
 *  {@link IOS_EXPORT_OPTIONS_PATH} already lives. */
export const ANDROID_BUILD_NUMBER_ARGS_PATH = 'android/.gradle/modoki-build-number.args';
export const IOS_BUILD_NUMBER_ARGS_PATH = 'ios/App/build/modoki-build-number.args';

/** The gradle twin of {@link xcodeBuildNumberArg}: `-PmodokiVersionCode=N` plus the init script that
 *  applies it to every application variant's outputs, so the committed `build.gradle` is never edited.
 *
 *  ⚠️ **Not `-Pandroid.injected.version.code`**, AGP's own property. It looks like exactly this and was
 *  tried first: on AGP 8.13 a command-line `assembleDebug bundleDebug` with it set to 424242 produced an
 *  APK and a bundle manifest both still at the committed 10125 (games/court, 2026-09-15) — AGP reads it
 *  only for builds the IDE invokes. The init script produced 424242 in both.
 *
 ...
 *  property — and inert without it, so a stale copy cannot change a build it was not asked to. */
export function renderAndroidVersionCodeInitScript(): string {
  return [
    '// Generated by the Modoki build (#1226) and rewritten on every native build — do not edit.',
    '// Hands -PmodokiVersionCode to every Android application variant without editing build.gradle.',
    "def modokiVersionCode = gradle.startParameter.projectProperties['modokiVersionCode']",
    'if (modokiVersionCode) {',
    '  gradle.beforeProject { project ->',
    "    project.plugins.withId('com.android.application') {",
    "      def components = project.extensions.getByName('androidComponents')",
    '      components.onVariants(components.selector().all()) { variant ->',
    '        variant.outputs.each { output -> output.versionCode.set(modokiVersionCode as Integer) }',
    '      }',
    '    }',
    '  }',
    '}',
...
/** A number pasted into a shell command: a positive integer, nothing else. */
function isInjectableBuildNumber(n: number | undefined): n is number {
  return Number.isInteger(n) && (n as number) > 0 && (n as number) < 2_100_000_000;
}

/** The DEBUG compile steps, kept beside the release ones so the build number (#1226) is visibly
 *  handed to all four — a debug install that silently carried the committed number would be the
 ...
 *  injected via env (not a bash export prefix).
 *
 *  `--no-daemon`: don't leave a persistent Gradle daemon (a java.exe running from the provisioned JDK)
 *  after the build. On Windows that daemon keeps the JDK's files LOCKED, so "Remove Java SDK" (and any
 *  manual delete) fails half-way. Small perf cost on repeat builds; worth it.
 *
 *  `clean` when `ota`: Gradle's incremental asset-merge task has been observed to miss a NEW file
 *  (ota-embedded-manifest.json) added to dist/ between builds, serving a stale merged-assets APK with
 *  no error (plan doc's "Gradle asset-merge staleness" gotcha) — costs a slower build only for
 *  OTA-enabled projects. */
export function androidDebugBuildStep(o: {
  androidCwd: string;
