...
export function createToolDef(server: McpServer, ctx: ToolContext): ToolDef {
  return (name, description, shape, handler) => {
    defineTool(server, ctx, name, description, shape, handler);
  };
}

/** The single implementation behind BOTH entry points — `createToolDef` (the static `modoki_*`
 *  groups) and the dynamic game-tool tail (#270, `gameTools.ts`).
 *
 *  It exists because the dynamic path needs the SDK's registration HANDLE back so it can
 *  `.remove()` a tool when the project changes, and `ToolDef` returns void. Writing a second
 *  registration path was the alternative, and it would have duplicated everything below — the
 *  strict schema, the §5 error stamping, the narrow-hint retarget — into a copy free to drift.
 *  A game tool gets exactly the same envelope as an engine tool because it runs exactly this
 *  code. */
