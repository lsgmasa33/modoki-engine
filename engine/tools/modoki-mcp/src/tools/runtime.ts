...
export function registerRuntimeTools(tool: ToolDef, ctx: ToolContext): void {
  ...
  tool(
    ...
    'Structured render/scene health report — turns "it renders black / looks wrong" into concrete ' +
      ...
      'capture_viewport). Returns {ok, summary, refs, transforms, camera, offScreen, uiOverflow, consoleErrors, ' +
      'errorWindowMs, olderErrors}. `uiOverflow` lists UI elements whose text painted outside its box ' +
      '(name, kind spill|own-box, boxName, overflowPx, clipped, text); a finding still `current` fails `ok`. ' +
      '`consoleErrors` is NOT an absolute: it holds only the errors ' +
