          ...
          groups: [
            ...
            },
            {
              ...
              fields: [
                ...
                { key: 'runtime.reloadOnDeadAudio', label: 'Reload when audio dies', type: 'checkbox', help: 'Reload the app when its audio is detected DEAD after returning from the background or an ad: the audio clock does not advance, even after a retry. Measured on iOS after another app (Apple Music) took the audio session: nothing short of a reload brings the sound back. The player sees the boot screen for a few seconds, and the game must persist its in-progress state, as for the setting above. Waits for the reload blockers (an ad on screen, plus whatever the game registers — Weaveling and Court block on a purchase and a sign-in) and re-checks the audio before reloading; at most one such reload per 10 minutes, so it cannot loop.' },
