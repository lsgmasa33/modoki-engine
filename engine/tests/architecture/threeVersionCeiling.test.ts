 ...
 *  has them ON, weekly and grouped — so its ignore is the one that DOES work, and it was MISSING
 *  until #1467: on
 *  2026-09-21 the grouped PR duly proposed three@0.186.0, refused only by the peer range one layer
 *  deeper. ⚠️ "Does work" means once SHIPPED: Dependabot reads the mirror's DEFAULT branch, which
 *  only a release publish moves, so a fix here is inert until the next release. This guard pins
 *  the SOURCE; it cannot tell you what the mirror is currently running.
 *  The case below asserts that entry exists and covers UNVERIFIED_FLOOR by RANGE.
...
const PRIVATE_DEPENDABOT = '.github/dependabot.yml';
const ossDependabotPath = () => path.join(REPO_ROOT, OSS_DEPENDABOT);

/** The npm ecosystem's `ignore:` entries, PARSED — never matched as text.
 *  ⚠️ The first version of this guard used `yaml.includes('>=' + UNVERIFIED_FLOOR)` and a regex
 *  `/dependency-name:\s*["']?three["']?/`. Review proved BOTH blind, with the entry still present:
 *  a NARROWED `">=0.186.0 <0.187.0"` passed (it contains the substring) — which is exactly the edit
 *  `HOW_TO_RAISE` forbids — and so did repointing the entry to `three-mesh-bvh`, because the closing
 *  quote was optional, leaving no `three` ignore at all. A guard that reads a STRING cannot see a
 *  RANGE; this one compares ranges with `semver.subset` and matches the name exactly. */
function ignoreEntries(rel: string): { name: string; versions?: string[] }[] {
  const doc = yaml.load(fs.readFileSync(path.join(REPO_ROOT, rel), 'utf8')) as {
    updates?: { 'package-ecosystem'?: string; ignore?: { 'dependency-name'?: string; versions?: string[] }[] }[];
  };
  const npm = (doc.updates ?? []).find((u) => u['package-ecosystem'] === 'npm');
  return (npm?.ignore ?? []).map((e) => ({ name: String(e['dependency-name'] ?? ''), versions: e.versions }));
}
...
describe('three version ceiling (#966, #957)', () => {
  ...
  it.skipIf(!hasOssOverlay())('the PUBLIC overlay bounds three by RANGE, not by a matching string', () => {
    expect(
    ...
    const three = ignoreEntries(OSS_DEPENDABOT).find((e) => e.name === 'three');
    expect(
      three,
      `${OSS_DEPENDABOT} has no ignore entry whose dependency-name is exactly "three". The mirror `
      + 'opens grouped minor/patch PRs weekly and three moves its MINOR every release (#1467).',
    ).toBeTruthy();
    const range = three?.versions?.[0];
    expect(range, `${OSS_DEPENDABOT}'s three ignore has no versions range`).toBeTruthy();
    // The ignore must cover EVERYTHING at or above the ceiling. A narrowed range (e.g. only the
    // one release known bad) leaves the next unverified release proposable — green under a
    // substring check, caught here.
    expect(
      semver.subset(`>=${UNVERIFIED_FLOOR}`, range!),
      `${OSS_DEPENDABOT} ignores three at "${range}", which does NOT cover everything from `
      + `${UNVERIFIED_FLOOR} up — so the next unverified release can still be proposed. `
      + `${HOW_TO_RAISE}`,
    ).toBe(true);
  });

  // ⚠️ The ignore SET is not part of the public/private divergence ruling (#1467) — a hold is on the
  // DEPENDENCY, and this is the only repo where the entries do work, because only here is the limit
  // raised. pixi.js and both Rapier packages were held privately and unguarded publicly for exactly
  // as long as nobody compared the two files.
  it.skipIf(!hasOssOverlay())('the overlay holds the SAME set of deps as the private config', () => {
    const names = (rel: string) => ignoreEntries(rel).map((e) => e.name).sort();
    expect(
      names(OSS_DEPENDABOT),
      `${OSS_DEPENDABOT} and ${PRIVATE_DEPENDABOT} hold different dependencies. The limit/schedule/`
      + 'groups MAY differ (that is #1467\'s ruling); the ignore set may not — the private entries '
      + 'are dormant and these are live. Carry the hold across, or lift it in both.',
    ).toEqual(names(PRIVATE_DEPENDABOT));
  });

  it.each(['@types/three'])('%s cannot walk past the ceiling alone', (pkg) => {
    // ⚠️ Created BY the three ignore: the grouped PR used to bump three and @types/three together
    // and die at install on the peer range. With three ignored, the types bump ALONE — r186 type
    // definitions against an r185 runtime, and nothing above sees it because DECLARATION_SITES
    // only reads `three`.
    const all = readJson('package.json');
    const declared = all.dependencies?.[pkg] ?? all.devDependencies?.[pkg];
    expect(declared, `package.json no longer declares ${pkg} — this guard has gone blind`).toBeTruthy();
    expect(
      semver.subset(declared, `<${UNVERIFIED_FLOOR}`),
      `package.json declares ${pkg} as "${declared}", which permits ${UNVERIFIED_FLOOR} and above `
      + `while three itself is bounded below it. Bound it the same way. ${HOW_TO_RAISE}`,
    ).toBe(true);
  });
