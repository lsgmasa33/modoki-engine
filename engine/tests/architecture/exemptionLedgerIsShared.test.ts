...
const RESIDUE: ReadonlyArray<{ item: string; count?: number; reason: string }> = [
  ...
  { item: 'engine/tests/architecture/posixLiteralComparison.test.ts::NORMALISED', count: 2, reason: 'CLASSIFIER — the sanctioned normalisations (#798 toPosix, plus the three legacy inline spellings docs/windows.md § Paths leaves alone); a comparison that normalises is not an occurrence of the banned pattern, so no offender is pardoned. Sibling of corpusConsumerPins::PINS. BOTH occurrences are the same classifier read by the two halves of one guard, and the second is safe for the same reason as the first: the corpus scan consults it, and the synthetic-source detector test consults it so that its reject cases assert the classifier rather than mock it — a detector test that skipped NORMALISED would pass while the real scan pardoned differently (#1435)' },
