...
const RESIDUE: ReadonlyArray<{ item: string; count?: number; reason: string }> = [
  ...
  { item: 'engine/tests/architecture/prefabSerializeCallSites.test.ts::CALL_SITES', reason: 'EXPECTED TABLE — a two-way CENSUS of every serializePrefab caller and where its file guid comes from (#1468). It pardons nothing: an undeclared caller FAILS, and so does a declared file that no longer calls it' },
