...
const WIDENED_LEDGER: ReadonlyArray<{ item: string; reason: string }> = [
  ...
  { item: 'engine/packages/modoki/src/editor/scene/prefab.ts::InstanceStructure.ownedNested',
    reason: 'scratch: same capture-and-consume call as InstanceStructure.consumedEcsIds — built by captureInstanceStructure from the same live query and read by serializeScene\'s reconcile pass (#1354). It IS an entity key, and it is held across the prefab-load awaits in that pass, but only alongside prefabRootInfo and nestedInstances, which key by the same live ids with the same lifetime; serialize spawns and destroys nothing, so a mutation able to recycle an id mid-pass would already corrupt those two. The revert filter passes it through unchanged and never keys off it (editor/scene/prefab.ts:2947); editor/scene/prefab.ts:1419' },
