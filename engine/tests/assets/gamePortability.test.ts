...
/** Generated/vendored native build output: gitignored, regenerated by Xcode/Gradle/`cap sync` on
 *  every build, and can be large (Pods, DerivedData) or hold third-party config with its OWN
 *  relative paths that would false-positive the native-config scan below. `walk()` above never
 *  needed this list — no project keeps `.ts`/`.tsx` source in these dirs — but `walkAll()` returns
 *  EVERY file, so without this it would walk straight into them. */
const skipNativeGenerated = (p: string) =>
  /[/\\](?:\.gradle|\.build|Pods|DerivedData|xcuserdata|Packages|build|capacitor-cordova-(?:android|ios)-plugins)[/\\]/.test(
    p,
  );
...
function walkAll(dir: string, out: string[] = []): string[] {
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    const p = path.join(dir, e.name);
    if (skip(p) || skipNativeGenerated(p)) continue;
