...
/** Re-exported from the MCP package so the gate and the #894 ledger cannot drift apart — see
 *  `engine/tools/modoki-mcp/surfaceBytes.ts` for the walk itself, which containers it descends
 *  into, and why property NAMES count. Kept exported here because this is where every existing
 *  consumer imports it from. */
export { sumSchemaBytes } from '../../tools/modoki-mcp/surfaceBytes';
