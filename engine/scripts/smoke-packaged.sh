...
PROJECT="$(cd "${1:-games/3d-test}" && pwd)"
OUT="/tmp/modoki-pkg-test"
APP="$OUT/mac-arm64/Modoki Editor.app"
BIN="$APP/Contents/MacOS/Modoki Editor"
VITELOG="/tmp/modoki-smoke-vite.log"; APPLOG="/tmp/modoki-smoke-app.log"
...
rm -rf "$OUT"
npm run build:electron >/tmp/smoke-build.log 2>&1 || { echo "[smoke] FAIL: build:electron"; tail -20 /tmp/smoke-build.log; exit 1; }
CSC_IDENTITY_AUTO_DISCOVERY=false ./node_modules/.bin/electron-builder --dir -c.directories.output="$OUT" >>/tmp/smoke-build.log 2>&1 \
  || { echo "[smoke] FAIL: electron-builder"; tail -20 /tmp/smoke-build.log; exit 1; }
[ -x "$BIN" ] || { echo "[smoke] FAIL: app not built"; exit 1; }
...
CONSOLE_ERR=$(curl -s -m 3 "http://127.0.0.1:$PORT/api/console-logs" 2>/dev/null | grep -iE "\[uncaught\]|\[unhandledrejection\]|error" | head -10)
if [ -n "$CONSOLE_ERR" ]; then echo "[smoke] WARN/FAIL: renderer console errors:"; echo "$CONSOLE_ERR" | sed 's/^/    /'; fail=1
