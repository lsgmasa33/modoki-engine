...
/** `appSupportRoot`, `SHARED_DIR` and `defaultToolchainDir` — re-exported from `toolchainHome.mjs`.
 *
 *  ⚠️ **Moved there in #1297, and this re-export is not a courtesy** — the same precedent as
 *  `altPathSpelling` below. `engine/toolchain/index.ts` now needs `defaultToolchainDir()` (a
 *  conversion resolves its pinned ffmpeg under it even when `MODOKI_TOOLCHAIN_DIR` is unset) and
 *  cannot import THIS module: the `REPO` const at the top throws in the bundled Electron main. The
 *  re-export keeps every caller and test on its existing import; the docs live with the
 *  implementation. Do not re-inline them here. */
export { appSupportRoot, SHARED_DIR, defaultToolchainDir } from './toolchainHome.mjs';
import { appSupportRoot } from './toolchainHome.mjs';
