...
for (const proj of process.argv.slice(2)) {
  ...
  for (const abs of files) {
    ...
    // already alien-animal.prefab.json at ~307,000 B — 35% of it, and up 2.5x in the
    ...
    try { old = execFileSync('git', ['show', `HEAD:${rel}`], { cwd: ROOT, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 }); }
      ...
      // long-committed prefab as untracked and SKIPPED ITS DIFF, so the review gate went blind on
